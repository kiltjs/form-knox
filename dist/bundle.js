...
function input (input, options) {
  ...
  var previous_value = input.value,
      mask_filled = null,
      custom_error = null,
      customError = options.customError || _noop$1,
      error_key = null,
      validation_message = '',
      listeners = { change: [] };
  ...
  function getErrorKey () {
    ...
    if( input.value ) {
      ...
      return customError( input.value, mask_filled, plainValue(input.value) ) || ( _inputMask && !mask_filled && 'uncomplete');
  ...
  function checkValidity () {
    ...
    runListeners$1(listeners.change, [plainValue(input.value), mask_filled, error_key, previous_value, validation_message ], input);
  }
  ...
  var component = {
    ...
    off: function (event_name, listener, use_capture) {
      if( listeners[event_name] ) _remove$1(listeners[event_name], listener);
...
formKnox.mask = inputMask;

var bundle = formKnox(formKnox, inputMask);
